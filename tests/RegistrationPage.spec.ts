...
let testCSVData = CsvHelper.readCsv('src/testdata/registration.csv');
for (let row of testCSVData) {
    test(`register to app with CSV data - ${row.username} - ${row.password}`, async ({ registrationPage, page }) => {
        await registrationPage.registration(row.firstName, row.lastName, row.email, row.telephone, row.password, row.passwordConfirm);
        expect(await registrationPage.getRegistrationPageTitle()).toBe('Your Account Has Been Created!');
        await page.pause();
    });
};
